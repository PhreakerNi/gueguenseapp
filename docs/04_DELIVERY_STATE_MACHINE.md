...
## 1. Ciclos de Vida Separados e Inmutabilidad de Cotizaciones
...
* **Regla de Inmutabilidad:** Cuando una entrega es cancelada, el `quote.status` **permanece INMUTABLE en `CONSUMED`** como evidencia histórica auditable. La penalización por cancelación o recargo se registra por separado en `pricing_adjustments`.
...
* **Terminal States:** `DELIVERED`, `RETURNED`, `CANCELED`, `FAILED`. (Las etapas `RETURN_REQUIRED` y `RETURNING` son transitorias de retorno, NO estados terminales).
...
| **SEARCHING_DRIVER** | **DRIVER_ASSIGNED** | Driver (vía Stored Proc) | Accept Offer | Driver active count = 0, Offer = `OPEN`. | Driver verified & active, `driver_presence` locked, GPS fresh. | `OFFER_ACCEPTED`, `DRIVER_ASSIGNED` | Offer = `ACCEPTED`, Presence = `BUSY`, Other offers = `CANCELED`. | Business Push/Socket | Obligatoria | `OFFER_EXPIRED`, `DRIVER_ALREADY_BUSY`, `DELIVERY_TAKEN`, `STALE_DRIVER_LOCATION` |
...
| **TO_PICKUP** | **ARRIVED_PICKUP** | Driver | Avisar Llegada | Delivery = `TO_PICKUP`. | Driver location within radius of business location. | `ARRIVED_PICKUP` | Backend generates `pickup_code_digest` in `private.delivery_secrets`. | Business Push/Socket | Opcional | `LOCATION_OUT_OF_RANGE` |
...
| **RETURN_REQUIRED** | **RETURNING** | Driver | Iniciar Retorno | Delivery = `RETURN_REQUIRED`. | Driver matches `driver_id` or authorized resolution. | `RETURN_STARTED` | Navigation to business location opened. | Business Socket | Opcional | `INVALID_DELIVERY_STATE` |
...
| **SEARCHING_DRIVER** | **CANCELED** | Business / Operator | Cancel Pre-Pickup | Delivery = `SEARCHING_DRIVER`. | Physical custody NOT transferred. | `DELIVERY_CANCELED` | Search aborted; quote status remains `CONSUMED`. | Fleet Socket | Obligatoria | `CANNOT_CANCEL_IN_TRANSIT` |
