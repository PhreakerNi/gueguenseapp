...
**Dominio:** Catálogo Relacional Completo PostgreSQL (34 Entidades), Esquema `private`, Secretos e Idempotencia  
...
* **Sensibilidad / Retención:** Sensibilidad media. Retención permanente.

#### 3. `public.business_members`
* **Propósito:** Relación de membresía N:M entre `auth.users` y `businesses`.
* **MVP Status:** MVP.
* **PK:** `id` UUID PRIMARY KEY DEFAULT gen_random_uuid().
* **FK:** `business_id` (FK `businesses.id` ON DELETE CASCADE), `user_id` (FK `auth.users.id` ON DELETE CASCADE), `location_scope` (FK `business_locations.id`, Nullable).
...
* **RLS:** Propietarios y gerentes del negocio administran sus miembros.
...
* **Sensibilidad:** PII Creada protegida. RLS restrictiva.
...
* **Propósito:** Estado geoespacial y operacional en vivo del conductor.
* **MVP Status:** MVP.
...
* **Propósito:** Registro y desglose detallado de la cotización calculada.
* **MVP Status:** MVP.
* **PK:** `id` UUID PRIMARY KEY DEFAULT gen_random_uuid().
...
* **Propósito:** Registro maestro del viaje y máquina de estados. (Sustituye pickup_code plano).
* **MVP Status:** MVP.
* **PK:** `id` UUID PRIMARY KEY DEFAULT gen_random_uuid().
...
#### 20. `public.delivery_tracking_points`
...
#### 21. `public.pricing_versions` & 22. `public.pricing_rules` & 23. `public.pricing_zones`
* `pricing_versions`: Versiones de la matriz de tarificación (`id`, `version_code`, `effective_from`, `is_active`). MVP.
* `pricing_rules`: Reglas de tarificación por distancia/tiempo (`id`, `version_id`, `base_fee`, `km_rate`, `min_fee`). MVP.
* `pricing_zones`: Polígonos geoespaciales PostGIS (`id`, `zone_name`, `polygon` GEOGRAPHY(Polygon, 4326), `surge_multiplier`). MVP.

#### 24. `public.pricing_adjustments`
* **Propósito:** Registros de recargos o descuentos sobre la entrega (`WAITING_FEE`, `RETURN_FEE`, `CANCEL_FEE`, `DISCOUNT`, `SUBSIDY`, `MANUAL_ADJUSTMENT`).
* **MVP Status:** MVP.
* **PK:** `id` UUID PRIMARY KEY DEFAULT gen_random_uuid().
* **FK:** `delivery_id` REFERENCES public.deliveries(id).
* **Columnas:** `adjustment_type` Text, `amount` Numeric(10,2), `currency` Text DEFAULT 'NIO', `reason` Text, `created_at` TIMESTAMPTZ DEFAULT NOW().

#### 25. `public.ledger_accounts`
* **Propósito:** Cuentas contables para partida doble.
* **MVP Status:** MVP.
* **PK:** `id` UUID PRIMARY KEY DEFAULT gen_random_uuid().
* **FK:** `user_id` REFERENCES auth.users(id), Nullable; `business_id` REFERENCES public.businesses(id), Nullable.
* **Columnas:** `holder_type` Text (`USER`, `BUSINESS`, `PLATFORM`), `account_category` Text (`ASSET_CASH_HELD`, `LIABILITY_DRIVER`, `ASSET_BUSINESS_REC`, `REVENUE_PLATFORM`, `BANK_PLATFORM`), `currency` Text DEFAULT 'NIO', `cached_balance` Numeric(12,2) DEFAULT 0.00, `created_at` TIMESTAMPTZ DEFAULT NOW().

#### 26. `public.ledger_transactions` & 27. `public.ledger_postings`
* `ledger_transactions`: Registro maestro de la transacción económica (`id`, `delivery_id`, `transaction_type`, `created_at`). MVP.
* `ledger_postings`: Entradas de débito y crédito (`id`, `transaction_id`, `account_id`, `amount`, `currency`, `created_at`). Regla: $\sum \text{amount} = 0$. MVP.

#### 28. `public.payments` & 29. `public.payouts` & 30. `public.cash_settlements`
* `payments`: Intentos de pago de comercios (`id`, `business_id`, `amount`, `payment_method`, `status`). MVP.
* `payouts`: Solicitudes de retiro de ganancias de conductores (`id`, `driver_id`, `amount`, `bank_account`, `status`). MVP.
* `cash_settlements`: Rendición de cuentas de efectivo cobrado en mano (`id`, `driver_id`, `expected_amount`, `settled_amount`, `status`). MVP.

---

### 2.6 Notificaciones, Soporte y Auditoría

#### 31. `public.device_tokens`
* **Propósito:** Tokens de dispositivos móviles (FCM / Expo Push).
* **MVP Status:** MVP.
* **PK:** `id` UUID PRIMARY KEY DEFAULT gen_random_uuid().
* **FK:** `user_id` REFERENCES auth.users(id) ON DELETE CASCADE.
* **Columnas:** `push_token` Text UNIQUE NOT NULL, `platform` Text, `last_seen_at` TIMESTAMPTZ DEFAULT NOW(), `is_active` Boolean DEFAULT true.

#### 32. `public.notification_outbox` & 33. `public.support_tickets` & 34. `public.audit_logs`
* `notification_outbox`: Registro Outbox para envío de notificaciones (`id`, `recipient_user_id`, `channel`, `payload`, `status`, `attempts`). MVP.
* `support_tickets`: Tickets de soporte operativo (`id`, `delivery_id`, `user_id`, `category`, `status`). MVP.
* `audit_logs`: Log inmutable de acciones administrativas sensibles (`id`, `admin_user_id`, `action`, `target_resource`, `reason`, `ip_address`). MVP.
