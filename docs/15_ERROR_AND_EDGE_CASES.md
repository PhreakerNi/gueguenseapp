...
3. **Listas Explícitas de Estados:** Se prohíben las comparaciones ordinales (ej: `status < PICKED_UP`). Las validaciones operativas evalúan listas explícitas de estados permitidos.
...
| **5. BUSINESS_CANCELS** | Clic cancelar en Negocio. | Valida si estado está en (`SEARCHING_DRIVER`, `DRIVER_ASSIGNED`, `TO_PICKUP`, `ARRIVED_PICKUP`). | Driver notificado de cancelación. | Permito pre-pickup $\rightarrow$ `CANCELED` | `DELIVERY_CANCELED` | Cobro `CANCEL_FEE` si aplica. | Proceso cerrado. |
| **6. CUSTOMER_CANCELS_IF_ALLOWED**| Solicitud vía soporte. | Evaluado manualmente por Operador. | Destinatario notificado. | Permito pre-pickup $\rightarrow$ `CANCELED` / `RETURN_REQUIRED` | `INCIDENT_OPENED` | Según política comercial. | Intervención operador. |
...
| **27. DRIVER_SUSPENDED_MID**| Conductor suspendido. | Rebotan nuevas ofertas; custodia se preserva.| Conductor deshabilitado. | Incidente obligatorio post-pickup. | `DRIVER_SUSPENDED` | Liquidación retenida. | Operador ordena RETURN / HANDOFF |
