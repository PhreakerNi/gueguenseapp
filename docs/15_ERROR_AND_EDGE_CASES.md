...
3. **Sin Pseudoestados ni Comodines:** Se prohíbe el uso de pseudoestados (ej. `PICKED_UP / DROPOFF`) y comodines (ej. `* -> RETURN_REQUIRED`). Únicamente los estados canónicos individuales (`PICKED_UP`, `TO_DROPOFF`, `ARRIVED_DROPOFF`) avanzan a devolución.
...
| **8. LOCATION_STALE** | > 3 min sin pings GPS. | Marca frescura tracking como `UNAVAILABLE`. | Alerta en mapa live. | Incidente en `incidents`. | `INCIDENT_OPENED` | Alerta preventiva. | Contacto telefónico Admin. |
...
| **27. DRIVER_SUSPENDED_MID**| Conductor suspendido. | Rebotan nuevas ofertas; custodia se preserva.| Conductor deshabilitado. | `PICKED_UP`, `TO_DROPOFF`, `ARRIVED_DROPOFF` $\rightarrow$ `RETURN_REQUIRED` | `DRIVER_SUSPENDED` | Liquidación retenida. | Operador ordena RETURN / HANDOFF |
...
| **30. CONTROLLED_HANDOFF** | Avería de moto en ruta. | Operador asigna 2do conductor (`custody_handoffs`).| Ambos firman traspaso. | Permanece `TO_DROPOFF` / `ARRIVED_DROPOFF` | `HANDOFF_STARTED` | Split de tarifa de servicio. | Traspaso presencial de paquete |
