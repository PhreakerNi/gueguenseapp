...
│ `INCIDENT_OPENED`, `INCIDENT_RESOLVED`, `INCIDENT_RESOLVED_ADMIN`,     │
│ `DRIVER_UNASSIGNED`, `DELIVERY_CANCELED`, `DELIVERY_FORCE_CANCELED`,   │
│ `DELIVERY_FAILED`, `QUOTE_CALCULATED`, `QUOTE_CONSUMED`, `QUOTE_EXPIRED`,│
│ `QUOTE_CANCELED`, `OFFER_EXPIRED`, `OFFER_REJECTED`, `OFFER_CANCELED`, │
│ `SEARCH_EXPANDED`, `MAPS_FALLBACK_USED`, `WAITING_STARTED`,            │
│ `OTP_ATTEMPT_FAILED`, `OTP_LOCKED`, `PAYMENT_INITIATED`,               │
│ `PAYMENT_AUTHORIZED`, `PAYMENT_CAPTURED`, `PAYMENT_FAILED`,            │
│ `PAYMENT_REFUNDED`, `DRIVER_SUSPENDED`, `DRIVER_REACTIVATED`,          │
│ `BUSINESS_SUSPENDED`, `BUSINESS_REACTIVATED`, `RETURN_AUTHORIZED`,     │
│ `CUSTODY_RETURNED`, `DRIVER_REGISTERED`, `DRIVER_VERIFIED`,            │
│ `DRIVER_REJECTED`, `DOCUMENT_SUBMITTED`, `VEHICLE_REGISTERED`,         │
│ `AVAILABILITY_CHANGED`, `PAYOUT_METHOD_ADDED`, `PAYOUT_METHOD_DISABLED`,│
│ `PAYOUT_REQUESTED`, `PAYOUT_APPROVED`, `PAYOUT_REJECTED`,              │
│ `CASH_SETTLED`, `BUSINESS_CREATED`, `LOCATION_ADDED`, `MEMBER_ADDED`,  │
│ `MEMBER_REMOVED`, `SUPPORT_TICKET_CREATED`, `PRICING_VERSION_ACTIVATED`.│
└────────────────────────────────────────────────────────────────────────┘
