...
SELECT is_definer('public', 'list_business_deliveries', ARRAY['uuid', 'uuid', 'uuid', 'text', 'integer', 'timestamp with time zone', 'uuid'], 'list_business_deliveries is SECURITY DEFINER');
...
SELECT function_privs_are('public', 'list_business_deliveries', ARRAY['uuid', 'uuid', 'uuid', 'text', 'integer', 'timestamp with time zone', 'uuid'], 'service_role', ARRAY['EXECUTE'], 'service_role has EXECUTE on list_business_deliveries');
SELECT function_privs_are('public', 'list_business_deliveries', ARRAY['uuid', 'uuid', 'uuid', 'text', 'integer', 'timestamp with time zone', 'uuid'], 'anon', ARRAY[]::TEXT[], 'anon lacks EXECUTE on list_business_deliveries');
SELECT function_privs_are('public', 'list_business_deliveries', ARRAY['uuid', 'uuid', 'uuid', 'text', 'integer', 'timestamp with time zone', 'uuid'], 'authenticated', ARRAY[]::TEXT[], 'authenticated lacks EXECUTE on list_business_deliveries');
...
VALUES
    ('a5000000-0000-4000-8000-000000000001', 'owner_f5_test@gueguense.com'),
    ('a5000000-0000-4000-8000-000000000002', 'mgr_f5_test@gueguense.com'),
...
VALUES
    ('c5000000-0000-4000-8000-000000000001', 'b5000000-0000-4000-8000-000000000001', 'F5 Central', 'Managua Centro', extensions.ST_SetSRID(extensions.ST_MakePoint(-86.25, 12.13), 4326), true),
    ('c5000000-0000-4000-8000-000000000002', 'b5000000-0000-4000-8000-000000000001', 'F5 Sucursal', 'Carretera Masaya', extensions.ST_SetSRID(extensions.ST_MakePoint(-86.22, 12.10), 4326), true)
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.business_members (id, business_id, user_id, role, status)
VALUES
    ('d5000000-0000-4000-8000-000000000001', 'b5000000-0000-4000-8000-000000000001', 'a5000000-0000-4000-8000-000000000001', 'business_owner', 'ACTIVE'),
    ('d5000000-0000-4000-8000-000000000002', 'b5000000-0000-4000-8000-000000000001', 'a5000000-0000-4000-8000-000000000002', 'business_manager', 'ACTIVE')
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.business_member_locations (business_member_id, business_location_id)
VALUES
    ('d5000000-0000-4000-8000-000000000002', 'c5000000-0000-4000-8000-000000000001')
...
INSERT INTO public.delivery_requests (
    ...
    '{"address_text": "Managua Centro", "latitude": 12.13, "longitude": -86.25, "location_id": "c5000000-0000-4000-8000-000000000001", "name": "F5 Central"}'::jsonb,
...
SELECT lives_ok(
    $$ SELECT public.verify_delivery_creation_scope('a5000000-0000-4000-8000-000000000001'::uuid, 'f5000000-0000-4000-8000-000000000001'::uuid) $$,
...
WHERE quote_id = 'f5000000-0000-4000-8000-000000000001';
...
WHERE delivery_id = (SELECT id FROM public.deliveries WHERE quote_id = 'f5000000-0000-4000-8000-000000000001');

-- 6.3 Test Append-Only Enforced on delivery_events (UPDATE & DELETE must fail)
SELECT throws_matching(
    $$ UPDATE public.delivery_events SET event_type = 'MUTATED' WHERE delivery_id = (SELECT id FROM public.deliveries WHERE quote_id = 'f5000000-0000-4000-8000-000000000001') $$,
    'APPEND_ONLY',
    'Updating delivery_events is strictly blocked by trigger'
);

SELECT throws_matching(
    $$ DELETE FROM public.delivery_events WHERE delivery_id = (SELECT id FROM public.deliveries WHERE quote_id = 'f5000000-0000-4000-8000-000000000001') $$,
    'APPEND_ONLY',
...
DECLARE
    ...
    v_res JSONB;
BEGIN
    SELECT id INTO v_del_id FROM public.deliveries WHERE quote_id = 'f5000000-0000-4000-8000-000000000001';
